...
export const LogoutDialog = ({ isOpen, onClose, onConfirm }: Props) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      {/* Modal Dialog */}
      <div className="relative w-full max-w-sm bg-white dark:bg-[#1E293B] border border-gray-200 dark:border-slate-700 rounded-2xl shadow-2xl overflow-hidden transform transition-all duration-200 scale-100 p-6 mx-4">
        ...
        <div className="flex justify-end pt-4 border-t border-gray-200 dark:border-slate-700 gap-3">
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm font-semibold border border-gray-300 dark:border-slate-750 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-805 transition"
          >
            Cancel
          </button>
          ...
          <button
            onClick={onConfirm}
            className="px-4 py-2 text-sm font-semibold bg-red-600 hover:bg-red-700 text-white rounded-xl shadow-md transition"
          >
            Logout
          </button>
        </div>
